...
async fn the_metrics_body_exposes_exactly_these_series_in_exactly_this_order() {
    ...
    // The hold histogram's stretch is built rather than written out
    // (ADR-159). With the labels stripped, every one of its 120 lines is the
    // same word, and it is the `holder` label — which this test drops on
    // purpose — that tells them apart: ten lines per holder, seven bounds
    // plus `+Inf`, the sum and the count. The labels themselves, and their
    // order, are pinned byte for byte by the golden in `metrics.rs`, which
    // is the right place for them.
    let hold = kimmy_storage::WriterHolder::ALL.iter().flat_map(|_| {
        std::iter::repeat_n(
            "kimmy_write_lock_held_seconds_bucket",
            kimmy_storage::WRITER_HOLD_BUCKETS_US.len() + 1,
        )
    ...
    .chain([
        ...
        "kimmy_sync_pull_seconds_bucket",
        "kimmy_sync_pull_seconds_bucket",
        "kimmy_sync_pull_seconds_bucket",
        "kimmy_sync_pull_seconds_bucket",
        "kimmy_sync_pull_seconds_bucket",
        ...
        "kimmy_sync_pull_seconds_bucket",
        "kimmy_sync_pull_seconds_bucket",
        "kimmy_sync_pull_seconds_sum",
        "kimmy_sync_pull_seconds_count",
        "kimmy_sync_pull_seconds_bucket",
        "kimmy_sync_pull_seconds_bucket",
        "kimmy_sync_pull_seconds_bucket",
        "kimmy_sync_pull_seconds_bucket",
...
async fn a_write_that_cannot_get_the_writer_in_time_is_a_503_timeout() {
    ...
    let hold = server.state.engine.hold_writer(kimmy_storage::WriterHolder::Bulk);
    let started = std::time::Instant::now();
    let refused = server
