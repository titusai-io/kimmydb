...
mod tests {
    ...
    /// on `n` when asked.
    fn seeded(state: &SharedState, coll: &str, count: i64, indexed: bool) {
    ...
    fn an_indexed_and_an_unindexed_source_give_the_same_answer() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let state = live_state(&dir);
        seeded(&state, "indexed", 40, true);
        seeded(&state, "plain", 40, false);
        let auth = superuser();
