...
|---|---|---|
| The server — `kimmyd` and the crates it is built from (`kimmy-core`, `kimmy-storage`, `kimmy-query`, `kimmy-vector`, `kimmy-auth`, `kimmy-cluster`, `kimmy-api`, `kimmy-mcp`, `kimmy-task`) and the `kimmy` CLI | [GNU Affero General Public License v3.0](LICENSE) | Free to run, self-host, and modify — personally or at a business. If you modify it and let others use it over a network, you must make your modified source available under the same license. |
| The client libraries — `kimmy-client` (Rust), `clients/go`, `clients/python` | [Apache License 2.0](LICENSE-APACHE) | Use them in any application, open or closed, with no obligation beyond keeping the license notice. |
