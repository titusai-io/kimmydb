...
| `kimmy_sync_ddl_applied_total{via}` | Replicated schema changes this node applied, by how they arrived: **`pull`**, in a window this node pulled from a peer, or **`push`**, in a window a peer pushed to it to confirm an index create or drop ([ADR-140](decisions.md)). It counts **entries applied, not entries received**: a refused, declined or skipped entry is on its own series instead. **A change already held here is counted again** each time a window carries it and the apply takes it as applied. That covers a creation of a collection that stands, and a definition or drop that is history. Those applies change nothing, but each is a real apply under the single writer, and they are what this series is for: a push carries everything the member lacks from the pusher ([ADR-143](decisions.md)), so each of N concurrent creates on one member is confirmed by a window re-carrying the ones before it, and each peer applies about N²/2 of them through `push` rather than N. Counted as each apply commits, whether or not the round or push then fails ([ADR-177](decisions.md)); a snapshot restore does not count here |
