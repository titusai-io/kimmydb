...
| `kimmy_write_lock_held_seconds_max` | The longest any one transaction has held the writer since start. Every other write on the node waited behind it. A value above a few seconds on a member with no bulk traffic is a background pass that walked more than it should have |
...
count rises once per round that did not run it — truncated out of it, or
failed ([ADR-145](decisions.md)). During a rolling upgrade, members still
...
```bash
V=0.26.1; A=kimmy-cli-x86_64-unknown-linux-musl.tar.xz
...
gh attestation verify oci://ghcr.io/titusai-io/kimmydb:0.26.1 -R titusai-io/kimmydb
```
