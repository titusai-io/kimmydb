...
| `kimmy_storage_cache_bytes` | Bytes the storage engine's page cache holds now: the **read cache and the write buffer together**, from redb's own count. Bounded by `storage.cache_bytes`, and filled by reads and never released on a timer, so it climbs to the busiest period's working set and stays there. **Read it against `storage.cache_bytes` and `kimmy_process_resident_bytes`**: this is the one part of the resident figure the cache accounts for. A step in resident memory **with** this gauge stepping by the same amount is the cache filling, bounded and expected. A rise in resident memory **with this gauge flat** is not the cache: it is heap (anonymous memory), which is the growth to chase. This is the reading that tells an idle step from heap growth: in round 0410, before this gauge existed, the growth that tracked the `/violations` route had to be read from the kernel's split instead — anonymous memory rising while file-backed memory stayed flat |
| `kimmy_storage_cache_evictions_total` | Pages the page cache dropped to make room, since the database was opened. Flat while the cache is below `storage.cache_bytes`. Rising while `kimmy_storage_cache_bytes` sits at the bound means the working set is larger than the cache: reads that miss go to the file, which `kimmy_write_lock_held_component_seconds_total{component="read"}` and the miss count show as cost |
