...
| `kimmy_task_retries_total{task}` | Times a supervised background task retried its work in place after a transient failure (`kimmy.task.retries` on the OTLP bridge). Every supervised task has a row from the first scrape, at 0. A **rising** count is the node recovering by itself and needs no action; a count that **keeps** rising while that task's work does not progress is a task retrying something permanent — alive, and doing nothing. That is the one case this series cannot tell you on its own, so read it beside the task's progress age rather than alone ([ADR-184](decisions.md)) |
...
```bash
V=0.33.0; A=kimmy-cli-x86_64-unknown-linux-musl.tar.xz
...
gh attestation verify oci://ghcr.io/titusai-io/kimmydb:0.33.0 -R titusai-io/kimmydb
```
