    ...
    spec:
      ...
      containers:
        - name: kimmy
          ...
          # run until this succeeds. The first start after an upgrade, a
          # restore or a migration walks the retained oplog (later starts skip
          # it): about 46 s per 10 million entries (the writes of
...
| `kimmy_oplog_verified_walk_seconds` | How long that walk took, warm or cold as it happened to be. 0 with no record |
