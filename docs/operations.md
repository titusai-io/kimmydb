...
| `kimmy_sync_ddl_applied_total{via}` | Replicated schema changes this node applied, by how they arrived: **`pull`**, in a window this node pulled from a peer, or **`push`**, in a window a peer pushed to it to confirm an index create or drop ([ADR-140](decisions.md)). It counts **entries applied, not entries received**. A refused, declined or skipped entry is not counted here. Neither is a replayed drop this node had already recorded, or a change for a collection dropped here (counted only as superseded); no series counts those two. **A change already held here is counted again** each time a window carries it and the apply takes it as applied: a creation of a collection that stands, or a definition or drop that is history. Those applies change nothing, but each is a real apply under the single writer, and they are what this series is for. A push carries everything the member lacks from the pusher ([ADR-143](decisions.md)), so when N concurrent creates on one member race their reads of what a peer holds, each may be confirmed by a window that re-carries the ones before it, and each peer can apply **up to about N²/2** of them through `push` rather than N. Windows also carry other members' earlier changes, so read this series as the **increase over a burst**, compared between builds, not as an absolute. Counted as each apply commits, whether or not the round or push then fails ([ADR-177](decisions.md)); a snapshot restore does not count here |
...
```bash
V=0.37.0; A=kimmy-cli-x86_64-unknown-linux-musl.tar.xz
...
gh attestation verify oci://ghcr.io/titusai-io/kimmydb:0.37.0 -R titusai-io/kimmydb
```
