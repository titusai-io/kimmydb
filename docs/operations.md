...
|---|---|---|
| `replication` | `kimmy_replication_lag_seconds` and `kimmy_sync_divergent_collections`. Not `kimmy_sync_peers_backing_off`, which the loop writes on every tick, failed rounds included, so it stays live exactly when rounds are failing and this age is climbing | 3 × `cluster.sync_interval_secs`: a round completes once a tick while any peer answers |
| `stall_probe` | `kimmy_runtime_stall_seconds` | 1 s, the stall alert's own threshold: the probe wakes every 250 ms, so an age above 1 s is a stall the gauge was not there to see |
...
| `embedding_worker` | the `kimmy_embed_*` counters: flat is only "nothing to embed" while this is fresh | 130 s: 2 × (5 s between idle turns + 60 s, the provider call's timeout) |

For the embedding worker, **a provider outage does not hold this age up on its
own**. A streamed batch whose provider call fails is left for a rescan, and the
worker goes on to its next turn; that shows as `kimmy_embed_failures_total`
climbing. What holds the age up is a retry loop, and there are three:

- a storage error the worker retries in place, with
  `kimmy_task_retries_total{task="embedding_worker"}` rising;
- the backfill after a vector configuration change, which retries its provider
  calls until they succeed;
- a re-check of a document another member wrote and did not embed, retrying
  its provider call. Documents merely waiting out that member's grace period
  do not hold it up.
