...
| `kimmy_task_progress_age_seconds{task}` | Seconds since a background writer last completed its work, computed when the page is read (one instrument per writer on the OTLP bridge, `kimmy.task.progress_age.<task>`). Four writers have one: `replication` (a round with a peer completed and measured the lag), `stall_probe` (the probe woke), `webhook_dispatcher` (a pass read the registry and set the backlog) and `embedding_worker` (a flush committed, or an idle turn had nothing waiting). **Before a writer's first completion it reads the time since the process started, never 0**, so a writer that has never run, one that has died, one that is stuck and one retrying something permanent all read the same way: old. **A writer this node does not run has no row**: `replication` with clustering off, `embedding_worker` with the worker disabled. The rows are fixed at startup, so the set does not change between scrapes. **Alert on this**, at a few multiples of the writer's cadence: `cluster.sync_interval_secs` for `replication`, a few seconds for the others. See [below](#a-gauge-is-only-as-fresh-as-its-writer) for which gauges each age covers ([ADR-187](decisions.md)) |
...
| `kimmy_webhook_subscriptions{state}` | Registered subscriptions, counted from this node's registry when the page is read: `active`, or `invalidated` after falling too far behind ([Webhooks](webhooks.md)). A record the dispatcher cannot load (one missing a field) is counted as `active`: it is registered, even though nothing delivers it. If the registry cannot be read, the scrape fails rather than reporting 0 ([ADR-187](decisions.md)) |
...
| Writer, by its age row | The gauges it sets |
|---|---|
| `replication` | `kimmy_replication_lag_seconds`, `kimmy_sync_peers_backing_off`, `kimmy_sync_divergent_collections` |
| `stall_probe` | `kimmy_runtime_stall_seconds` |
| `webhook_dispatcher` | `kimmy_webhook_backlog_seconds` |
| `embedding_worker` | the `kimmy_embed_*` counters: flat is only "nothing to embed" while this is fresh |
...
```bash
V=0.34.1; A=kimmy-cli-x86_64-unknown-linux-musl.tar.xz
...
gh attestation verify oci://ghcr.io/titusai-io/kimmydb:0.34.1 -R titusai-io/kimmydb
```
