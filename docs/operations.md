...
| `kimmy_sync_serve_failures_total{reason}` | Replication connections a peer opened to this node that ended in an error **on this side**, by reason: **`io`**, reading or writing the connection failed — a peer that went away mid-exchange, such as a pusher that stopped waiting (the `Broken pipe` and `Connection reset` WARNs); **`timeout`**, the handshake ran out of time; **`malformed`**, a frame this node could not read or would not accept, or a local error answering one; **`unauthenticated`**, the peer failed the shared-secret proof; **`fault`**, the peer reported a fault of its own; **`binding`**, the TLS session gave no channel binding and the peer was refused. A clean close between requests is how every contact ends, and is not counted; a TLS handshake that never completes is not counted either, because anything that can reach the port can cause one. Each counted failure is also logged at `WARN` as `peer connection failed`, naming the peer and the reason. `kimmy_sync_failures_total` is the other side: rounds this node **pulled** that failed. A steady rise in `io` alongside `kimmy_ddl_confirmations_total{outcome="failed"}` on a peer is pushes giving up on this node; `unauthenticated` is a member with the wrong `cluster_secret`, or something that is not a member |
