...
**Amended 2026-09-07 — the actions dist generates are exempt from major
bumps.** `.github/workflows/release.yml` is generated by dist and checked for
drift by the `plan` job on every pull request (ADR-063). Dependabot's grouped
GitHub Actions update edited it — `actions/checkout` v6 to v7 in six places —
and `dist plan` refused the pull request, correctly: dist 0.32.0 generates
v6, and no newer dist generates v7. Dependabot has no per-file exclusion for
the GitHub Actions ecosystem, only `ignore` by action name, and dist's
`allow-dirty = ["ci"]` is the knob its own documentation says not to reach
for, since it stops dist maintaining the file. So `dependabot.yml` now
ignores **major** updates for the four actions dist writes into that file
(`actions/checkout`, `actions/upload-artifact`, `actions/download-artifact`,
`Swatinem/rust-cache`). Minor and patch bumps still arrive; a major moves
when a dist upgrade regenerates `release.yml`, and the hand-written
workflows are moved to the same major by hand in that change. The cost is
that `ci.yml`, `deny.yml`, `fuzz.yml` and `publish-ghcr.yml` no longer get
those four majors proposed automatically, which is a handful of edits once
or twice a year against a weekly pull request that cannot merge.
...
**That each of those two rules has a single site is the point of this record,
...
That last sentence is literal, and the dependency is worth stating because it is
invisible from either half on its own. The rule below — a resumed page carrying
the sender's drop, so a receiver part-way through a repair drops the partial
copy — works **only** because the collection it is about to drop was created
under the sender's stamp. Under the old local-clock origin, a receiver working
through a backlog holds a `created` later than the sender's drop, so
`aims_at_a_previous_incarnation` reads that drop as belonging to a life that
ended before its own copy began, ignores it, and keeps the partial copy: the
same outcome as sending no drop at all. The two are correct only together, and
no test sees it, because a fixture's engines run in real time and never open the
gap a backlog does.
...
*A scoped snapshot carries the sender's drop on **every** page, not the first
alone.* A repair runs for as long as the collection takes, and a drop lands
where it lands. Sent on the first page only, a drop between pages reached the
receiver as a page with no definition, no documents, no drop and no cursor —
which is exactly what a snapshot that has simply run out looks like — so the
pull reported itself complete and the receiver kept a partial copy of the
incarnation the cluster had just agreed to delete, with no tombstone of its own
to stop it serving or re-seeding that copy. `Engine::snapshot_page` now matches
on the scope first and lets the cursor decide only whether the *definition*
rides along; the drop question is asked on every scoped page. Measured on a
live-shaped fixture rather than inferred: 512 documents applied, the sender
drops, the resumed page comes back empty, the receiver still holds 512. It is
also the route the receiver-side rule below was written for — without this it
was unreachable there, while the original round's logs show that route firing
dozens of times.

*And the sender asks whether the collection is **gone** here, not whether a
tombstone exists.* A collection recreated after a drop keeps that drop's
tombstone, because the tombstone is what floors the new incarnation (ADR-081) —
so "a tombstone exists" does not mean "the collection is gone". A page that read
the tombstone alone would carry a drop while the sender still held the
collection, and a receiver at an older incarnation would then destroy the very
copy the snapshot was filling. That arm of the match does nothing, which is
precisely why it has to be there: it is what keeps this fix from becoming a new
loss path of its own.
...
One asymmetry between them survives, and is named here so it is not later
rediscovered as a defect: on the **ignore** branch the entries path records the
tombstone anyway and the snapshot path does not. Both reach that branch through
the same predicate and agree about the answer; only what they do afterwards
differs. `apply_ddl` records because it is holding an entry it has decided not
to apply and must not re-serve, and the tombstone never moves backwards, so a
straggling pre-drop write is superseded here as it would be anywhere else. The
snapshot route is holding no entry, and a tombstone below the `created` of the
incarnation standing here says nothing that incarnation's own floor does not.
...
**The resumed page adds no field and changes no shape**, which is worth saying
because it looks like a wire change and is not. `SnapshotPage::dropped` already
existed and was already `#[serde(default)]` (ADR-152); what changed is *when a
sender fills it*. A receiver that predates the change reads a resumed page's
drop exactly as it reads a first page's, because its gate was always on the
snapshot being scoped and never on the page being the first.

**Its closure condition is both ends upgraded, not "after the roll".** The
sender must be new for the drop to be sent on a resumed page at all, and the
receiver must be new for that drop to be *acted on* when it holds the
collection: at 0.26.0 `restore_collection_drop` returns before recording the
tombstone in exactly the case this creates — a receiver holding a partial copy —
so an un-upgraded receiver gains nothing here from an upgraded sender. It also
loses nothing. It behaves as it did: the drop is ignored, no tombstone is
written, no round fails, and the partial copy stands as it would have with no
drop on the page. So during a roll this hole closes for a pair only once both
...
and one field on two `INFO` lines. No new `/metrics` series, and no third wire
field for the resumed page — `SnapshotPage::dropped` is the one ADR-152 already
put there. A divergence answer grows by an id and an `Hlc` per collection the
answering node holds, on a message that already carried the ids. A resumed
scoped page costs one `collection_by_id` lookup it did not make before, against
a page that walks a key range.
...
**A snapshot does not reconcile a differing incarnation**, and that is the
second open item, pre-existing and tracked with the first. A scoped page carries
the sender's drop only while the collection is *gone* there — correctly, for the
...
away; by the pair that pins the resumed page —
`a_scoped_snapshot_resumed_after_its_sender_dropped_the_collection_carries_the_drop`
(a page applied, the sender drops between pages, and the resumed page takes the
receiver's partial copy with it, at the sender's stamp and minting no entry of
the receiver's own; it also asserts the whole-database route still carries no
drop, which is the residual and not this fix) and
`a_resumed_scoped_page_of_a_collection_recreated_after_a_drop_carries_no_drop`,
the arm that must do nothing — a surviving floor is not news, and a page that
read the tombstone alone would tell a receiver to destroy what it was
filling; by `kimmy-cluster`'s `divergence_crosses_a_version_boundary_in_both_directions`
...
> A served window now passes over what the puller's witnessed vector covers, so a
> held entry is re-served only while it is above the member's witnessed position
> for its origin.
> - **Before.** A held entry at or below that position, such as a scoped repair
>   filling a hole, was re-served and released whenever another origin held the
...
window the same way. The cost stays on the sender. The range from the threshold
is still walked, in one read transaction, until the first entry the requester
lacks. Each entry passed over costs a key decode rather than a body decode, but
redb still reads the leaf pages holding the values. So a pull over an oplog the
requester mostly holds is one long walk rather than hundreds of frames. The walk
runs under `kimmy_storage::blocking` (ADR-153) in the served arm and in the push,
which walks it twice after `BatchTooLarge`. It is not capped: a cap would produce
an empty window that is not a tail, which ADR-126 makes malformed.
...
a measurement: for the probe above both options send one entry. A walks up to
about 313,000 oplog rows once, reading their pages and decoding only their keys.
That was not measured, and on a cold page cache it runs at the disk's speed
(ADR-151). B, with a new index, reads one row and pays an extra index write on
every oplog append on every member, forever. Mixed-version, B needs the requester to fall back to one
...
origin. Every entry passed over is at or below `held[origin]`: the requester had
processed it when it sent the request, or holds coverage of it. That coverage is
either a snapshot's grant (ADR-082) or the state-only coverage ADR-167 stands
for. Its witnessed vector only rises while the window is in flight, so nothing
passed over becomes something it lacks by this change.

What the skip gives up is incidental re-delivery of entries the witnessed vector
covers but the requester does not hold in position. There are two kinds. One is
...
**ADR-169's release, which the skip narrows.** An entry held as state (ADR-160)
is applied under `Position::Hold`, which raises neither vector. So a held entry
above the requester's witnessed position for its origin is still served, and
ADR-169 releases it on arrival. That covers the whole-database snapshot's case,
a document written behind the cursor above the grant, which is the case the live
round exercised.
...
- **After it.** S is never re-served while R's witnessed vector covers it. The
  mark goes in one of two ways:
  - O writes again and R appends in position past S. That raises R's servable
    vector over S, and the mark is left to matter only to ADR-160's open-time
    rebuild, which merges and never lowers. **The confinement to that rebuild
...
ADR-169's consequence that "a scoped repair's marks now go when their entries
arrive in position" is corrected accordingly: they go that way only while the
entry is above the member's witnessed position. The lowered-vector fix for this
residual is weighed under *Alternatives* and not built.
...
- *Lower `held` below the member's lowest held mark on each origin* (the
  ...
  - **It needs no new field.**
  - **It turns pulls into snapshots.** `held` is also the vector
    `can_serve_peer_holding` judges the horizon on (ADR-097). A position lowered
    below what the sender has collected of that origin is answered
    `BeyondHorizon`, and the round becomes a whole-database snapshot. A repair's
    marks sit at the stamps of the documents the member lacked, and a live
    document is routinely older than the sender's retained history of its
    origin. So in the case the marks come from, every pull would become a
    whole-database snapshot until one completed. The requester cannot avoid
    that, because it does not know what the sender has collected.
  - **A clean version needs a wire change.** It would send the re-serve floor as
...
- `a_version_roll_re_registers_every_member_without_re_serving_what_its_peers_hold`:
  the roll shape, through `topology::register` with a changed version, driven
  through the storage window rather than over TCP; red without the skip.
- `a_push_to_a_caught_up_member_carries_the_change_and_not_what_it_holds`: the
  push serves the skipped window; red without the skip.
...
**Decision.** `AskEntries` gains `marked`, with `#[serde(default)]`: a list of
spans, each `MarkedRange { origin, from, through }`.
- **What the requester names.** Per origin, the lowest and highest entry it
  holds under an `OPLOG_HELD` mark (ADR-160) that its witnessed vector covers
  (`Engine::held_ranges_covered_by`). The upper bound is clamped to what the
  peer advertises of that origin, and a span the peer advertises none of is
  left out. None are named beside a repair, whose replay sends no `held` and is
  served its range whole.
- **What the sender serves.** Every entry of that origin inside the span, in
  addition to what ADR-171's skip serves (`Engine::entries_for_peer_marked`).
  The sender starts its own scan at the lowest span when that is below `from`.
  Every entry below `from` is covered by `held`, so starting lower serves nothing
  outside a span.
- **The release.** A served entry arrives in a window, and ADR-169 releases its
  mark.
- **The request's `from` is never lowered for a span**
  (`transport::entries_threshold`).
  - When the requester is behind the peer, `from` is ADR-171's threshold.
  - When it is behind on nothing and names spans, `from` is the highest stamp
    the peer advertises. A sender that ignores `marked` then serves only the
    entries at or above that stamp.
- **A resume point per peer and origin** (`PeerStalls`). The span named on the
  wire starts where the last window from that peer left it:
  - after a window truncated at the cap, past that window's last entry, with
    the entry's full stamp deciding a tie, as the coverage rule's does (ADR-148);
  - after a window that reached the peer's tail, past the span's top. The span
    is then dropped from the ask. That peer has been walked across the whole
    span, so whatever is still held is an entry it provably lacks, and
    retention or another peer releases it.
  - A window stopped at a collection this node lacks moves nothing, and nor
    does an answer that is not a window.
- **When a span is asked from its bottom again.** Any of:
  - the member holds more marks on that origin than when the resume point was
    recorded;
  - the peer now advertises more of that origin than it did;
  - `MARKS_REASK_AFTER` (300 s) has passed since the resume point last moved.
    A peer can take the missing entry below its own position on an origin that
    has gone quiet, which moves nothing the requester can see; the five minutes
    bound how long that release waits.
...
- **Who sees it.** A peer whose witnessed vector covers the entry skips it and
  sees nothing. A peer that also lacks it, pulling from that member, is served it
  above the advertised vector and defers it as `beyond_advertised`. That peer
  takes the entry from any member that holds it in position.
...
- **Where it comes from.** The shape found is a hole filled by a scoped repair.
  A hole is an entry witnessed and never applied, the ADR-148 class. A
  whole-database snapshot's grant releases every mark it covers, and a mark
  above the grant is above the witnessed vector, which ADR-171 still serves. No
  path on a correct build that produces a mark at or below the witnessed
  position without a hole was found.
...
- **Measured.**
  ...
  puts ten marks in the middle of 4,196 entries of one origin, on a member that
  is behind on nothing. A floor at the lowest mark serves 2,196 entries, three
  windows, all but ten of them superseded. The span serves the ten, in one
  exhausted window. These are entry counts, fixed by the fixture.
...
**The failure the first form of this record had, found in review.** The first
form lowered the request's `from` to the lowest span on every pull. It served
every entry inside a span, held or not, counting towards the batch limit.
- **The trigger.** A peer that does not serve a span's lowest entry, but holds
  at least 1,024 of that origin's entries above it inside the span, pinned the
  pull. A peer does not serve that entry when it has collected it, when it
  shares the hole, when a rewind removed it, or when it is on an older build.
  The ADR-148 shape meets the 1,024-entry condition routinely.
- **What it did.** Every window was the same 1,024 superseded entries.
  - Nothing was released, the window was truncated, and the witnessed vector
    already covered all of it.
  - The next pull asked the same thing, up to 128 pulls a tick.
  - Nothing above the span was served from that peer, the divergence check never
    ran on that contact, and no failure counter moved.
  - It ended only when the requester's own retention collected the mark, or
    when a third member holding the entry served it first. A two-member cluster
    had no third member.
- **The two reproductions, over TCP with one `PeerStalls` for every pull:**
  - *The peer collected the span's lowest entry, then wrote* (x10 collected on
    A, R holding x10 and x2900).
  - *The peer shares the hole* (A witnessed past d10 without applying it, R was
    repaired from the origin, R holding d10 and d2900).
  - On the first form, both pulls repeated as applied 0, superseded 1,024 and
    truncated, six times out of six. The later write never arrived.
  - With the resume points, both finish in three pulls (1,024, 1,024 and 842
    superseded), that is ⌈2,890 / 1,024⌉ for the entries A holds inside the
    span, and the later write arrives on the third.
- **The fix.** The resume points move each span's bottom past what the peer
  served, so a span is walked once, a window at a time, and the window then
  reaches whatever lies above it. The span is dropped once a window reached the
  peer's tail across it. The request's `from` is no longer lowered, so an older
  sender is never asked to serve from a span's bottom.
...
- **A span below the sender's horizon.** It is served whatever the sender still
  has inside it. Entries the sender has collected do not arrive, and their marks
  stay until the requester's own retention collects them, or another peer
  serves them.
- **A span-bearing request answered `BeyondHorizon`** records no resume point.
  The whole-database snapshot that follows serves no span. Its grant releases
  every mark at or below the grant, in the transaction that adopts it
  (`release_held_under`). A covered mark above the grant is named again on the
  next pull.
- **Nothing is witnessed by a span.** The receiver's coverage rule is unchanged,
  ...
  it.
- **Why not lower `held` instead.** Lowering `held`, ADR-171's first form of this
  idea, would have the horizon judge the lowered vector. It would answer a
...
- **Per pull, on the requester.** A range read of `OPLOG_HELD` up to the
  ...
  - The table is not empty on every member. A member that completed a
    ...
    applied (the table's own note).
  - Marks above the witnessed position for their origin are read and left out.
  - Not measured.
- **Per span, on the sender.** A walk of that origin's entries inside the span
  that the sender holds, a window at a time: ⌈W / 1,024⌉ pulls for W such
  entries, once per resume point.
  - When the span's lowest entry is servable, it is in the first window and is
...
- **After a span is dropped.** Nothing, until one of the reopening conditions
  above holds. The three conditions cost differently:
  - **Marks grown** is rare: a repair or snapshot brought more.
  - **Record expiry** re-walks a span no peer can serve once every five minutes
    per peer, until retention collects the marks.
  - **The peer moving on the origin** is the expensive one. A peer that keeps
    writing to that origin, or keeps receiving its writes, moves on it every
    tick. If that peer still lacks the entry, the span is walked again every
    tick: ⌈W / 1,024⌉ extra pulls per tick per such peer, for as long as the
    marks live. That is bounded per tick, and replication beneath it progresses.
    It is the cost of reopening on the peer's movement, which is what covers a
    peer gaining the entry by restore, replay or its own release. A cheaper
    reopening, probing only the held entries' exact stamps, needs the marks
    themselves on the wire, and is not built.
...
`AskEntries`, empty when there are no spans, so a mixed-version pair crosses the
boundary on every pull.
- **A sender on 0.28.1** ignores `marked` and has no ADR-171 skip. It receives
  the ordinary ask. When the requester is behind, that is exactly what a 0.28.1
  requester sends, and it is served what 0.28.1 serves today, the ADR-171 drain
  included. When the requester is behind on nothing, a 0.28.1 requester sends
  nothing. A 0.29.0 requester holding spans asks from the peer's newest stamp,
  and is served the entries at that stamp and whatever arrived since the vector
  was read. Nothing inside a span is served, so those marks wait for a 0.29.0
  sender or for retention. (The first form of this record lowered `from`, which
  re-created the whole drain from the lowest mark on such a sender. That drain
  released the marks as a side effect.)
- **A sender on ADR-171 alone** skips and ignores `marked`. It serves what
  ADR-171 serves, and the marks wait the same way.
- **An older requester** sends none and is served as ADR-171 serves.
- **No ordering in the roll.**
- **Held by** `ask_entries_marked_crosses_a_version_boundary_in_both_directions`,
  which checks decoding only, and
  `a_requester_naming_spans_asks_a_sender_that_ignores_them_for_no_more_than_before`,
  which checks what such a sender is asked.
  - The older senders are simulated by the storage calls each build serves with:
    `entries_for_peer` for 0.28.1 and `entries_for_peer_holding` for ADR-171.
  - The threshold is computed by `entries_threshold`, which is what the round
    itself sends.
...
- *Stop naming spans for the rest of a contact after a truncated pull that
  released nothing.* It leaves a wasted pull per tick per peer for as long as
  the marks live, and never finishes walking a span wider than a batch.
...
- `a_held_entry_below_the_members_position_is_released_by_its_next_pull`: over
  TCP, read from the round's own log. Spans are named, there is no snapshot,
  it takes one pull, and the entry is released.
- `a_span_the_peer_has_collected_is_served_what_remains_and_never_a_snapshot`: a
  span below the sender's horizon.
- `a_span_whose_lowest_entry_the_peer_collected_does_not_stop_replication_from_it`:
  reproduction one. The later write arrives in three pulls, and the next round
  names nothing.
- `a_span_whose_bottom_the_peer_cannot_serve_does_not_stop_replication_from_it`:
  reproduction two, the shared hole. Three pulls, ⌈2,890 / 1,024⌉.
- `a_span_the_peer_answered_is_not_named_again_on_the_next_round`: the resume
  points' wiring across two rounds.
- `a_span_is_named_again_once_the_peer_has_moved_on_its_origin`: the re-ask.
- `a_requester_naming_spans_asks_a_sender_that_ignores_them_for_no_more_than_before`:
...
- `peer_stalls_resume_each_span_past_what_the_peer_served`,
  `the_spans_named_are_logged_when_they_change` and
  `a_pull_asks_from_its_threshold_and_never_from_a_span`: the resume points, the
  log and the threshold.
