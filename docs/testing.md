...
| **The node's shutdown announcement is covered at unit level only** | Removing both `shutdown.begin()` calls from `node.rs` leaves every end-to-end drain test green, because the drain aborts each supervised handle and an aborted supervisor is cancelled before it classifies anything. `a_task_that_ends_by_itself_during_shutdown_does_not_exit` is what fails, and it is a unit test. Measured, not assumed ([ADR-184](decisions.md)) |
