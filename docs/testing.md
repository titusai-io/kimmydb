...
| `KIMMY_TEST_FAIL_STORAGE` | `read`, `write`, `sync_data`, `set_len` or `len`, optionally with `@write` (`sync_data@write`), which fails the call only inside the commit of a client write, not a background writer's commit that might come first. Fails the next storage call of that kind once, with EIO, once the node is serving, so a test can assert that the node exits 70, and that the next start names `storage_failed` and repairs the file ([ADR-188](decisions.md)). **Every start where it is set logs a `WARN` naming it**, and a value that names no call says so |
