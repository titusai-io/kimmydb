...
| `kimmy` CLI | ✅ Done — one-shot subcommands over the HTTP API, JSON on stdout. An interactive shell was considered and deferred: it is the same command surface plus a terminal UI. Since moved to its own repository with the client libraries, frozen and not currently distributed ([ADR-193](decisions.md)). [CLI](cli.md) |
...
| 2 | Prove | ✅ **Latency histograms + oplog lag** | ADR-046: `kimmy_request_duration_seconds` with buckets **measured** on a release build (the first draft of the numbers was written from expectation and the measurement corrected it), and `kimmy_replication_lag_seconds` pushed from the replication loop — zero when caught up, holding its last value through an outage; first measured as the span of unapplied peer history by entry timestamps, which ADR-122 replaced with how far behind in time the node is. Found while measuring: `find {_id}` is a collection scan (the planner never consults the primary key) — recorded 🟡 in [Deviations](deviations.md) |
...
| 2 | ✅ **The error taxonomy becomes public surface** | `ErrorCode` is an enum, so the code set is closed by the compiler and a new one cannot be added without declaring its retry class in the same commit. **Retryability is three-valued** — `no` / `wait` / `elsewhere` — because leaderless replication makes "ask a different node" a real answer, and it rides in the envelope so a client handles codes released after it was written ([ADR-057](decisions.md)). Enumerating found `no_vectors` in no document and 422 specified nowhere |
...
| 7 | ✅ **An HTTP-level benchmark harness** | `cargo bench -p kimmyd --bench http` spawns the **shipped binary** and drives it with concurrent clients over a real socket, plaintext and TLS, reads and writes. TLS turns out to be close to free and the protocol costs ~0.1 ms per request; reads scale with clients and writes do not. It also found a gap nothing else could see — a write costs about twice as much through the daemon as at the engine — recorded as a question rather than an answer |
| 8 | ✅ **The Rust client — `kimmy-client`** | Auth with refresh, cursor iteration, change streams with reconnect and resume, typed errors carrying the retry class, and multi-node failover. **It depends on no `kimmy-*` crate**, so it sees what the Python and Go clients will see. `kimmy-cli` is a consumer of it — nothing there builds a URL or reads a status code — and the conversion found three real defects, including a public API that forced every consumer to depend on `reqwest`, and a login that could not fail over Since moved to its own repository, frozen and not currently distributed ([ADR-193](decisions.md)). |
| 9 | ✅ **The Python client** | `clients/python`, package `kimmydb`, synchronous on `httpx` + `websockets` — both have async APIs behind nearly the same surface, so async stays a decision rather than a rewrite. Shares no code with the Rust client and passes the same scenario list. Found that a lazy change stream misses everything written before the first read, and that **a dropped collection leaves a stream open and silent**, which is a server behaviour neither client can paper over Since moved to its own repository, frozen and not currently distributed ([ADR-193](decisions.md)). |
| 10 | ✅ **The Go client** | `clients/go`, package `kimmydb`, one dependency — `net/http` pools connections, so only WebSocket framing is missing from the standard library. `coder/websocket` because it handshakes through an ordinary `*http.Client`, so a stream inherits one TLS configuration rather than a second that can drift. Paging and streaming are range-over-function iterators, which makes the error the second loop variable rather than something to remember. Passes the same scenario list as the other two Since moved to its own repository, frozen and not currently distributed ([ADR-193](decisions.md)). |
| 11 | ✅ **A conformance suite every client must pass** | `clients/conformance`: one declared scenario list with the observations a correct client must produce, a driver per client, and a runner that starts a fresh node per scenario and compares. Checks coverage *and* behaviour — three per-language suites can each have a `failover` test and disagree about what failover means. Verified by breaking a client on purpose. Found on its first full run that the specification had claimed collection creation was idempotent since task 1, while the server has always returned 409 Its drivers have since moved with the clients, and this repository's CI no longer runs it; the scenarios stay as the protocol's contract ([ADR-193](decisions.md)). |
