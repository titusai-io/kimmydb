...
| 2 | **The error taxonomy becomes public surface** | Today errors are `{"error": "<code>", "message": "…"}` — the HTTP status carries the class, and the codes accreted per site: `duplicate_key`, `unique_violation`, `resume_token_expired`. Clients branch on these, so the code set becomes API: enumerate it, make it exhaustive over `ApiError`, and test that every constructor maps to a listed code. Task 1 lists the sixteen in use in `docs/openapi.yaml` and says plainly that closing the set is this task. **String tags are a genuine advantage over Mongo's numeric codes** and worth keeping. **Reserved**: which errors are declared *retryable*, since that decides what a client is allowed to retry automatically |
