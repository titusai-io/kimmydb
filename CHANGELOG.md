...
- **A restart no longer reads the whole oplog.** Every start walked every
  ...
  current the same way and ignore the record. [Operations](docs/operations.md#capacity)
  gives the start-time figures, and ADR-173's addendum of 2026-09-26 has the
  argument.
- **A change-stream resume no longer holds a request worker while it finds
  where to start, and a slow one is logged.** Resuming with a token another
  member issued walks this member's arrival index before the upgrade is
...
- **A change-stream resume on a member that did not issue the token no longer
  walks the whole retained oplog to find where to start.** The member races a
  seek by stamp against the walk, and the first to finish answers, which costs
  at most about twice the cheaper of the two. A client that was caught up with
  the member reads at most about twice the member's state marks
  (`kimmy_sync_held_marks`), and nothing when there are none. Where the stream
  starts is unchanged. When a slow resume is logged, the line now also carries
  `found_by`. The start-up check that rebuilds the arrival index now covers both
  of its halves; when only the stamp half is short, it is rebuilt without
  moving any position. ADR-173's addendum of 2026-09-26.
...
- **`kimmy_task_progress_age_seconds{task}`** (`kimmy.task.progress_age.<task>`
  on the OTLP bridge): seconds since each background writer last completed its
  work, for `replication`, `stall_probe`, `webhook_dispatcher` and
