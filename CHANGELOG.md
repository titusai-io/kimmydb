...
A patch: no wire, storage-format or API break; rolling upgrade. Three limits
an operator can set on the authenticated routes, so a principal that is
compromised or a client with a broken retry loop cannot hold a node hostage.
Each defaults to what the server already did — the body ceiling is the one
axum always applied, the deadline is far above any measured latency, and the
per-principal limit is off — so a node that sets none of them behaves exactly
as before (ADR-099).
